...
class Site:
    ...
    def _occ_view(self, r: sqlite3.Row) -> dict:
        ...
        return {
            ...
            "title": r["title_raw"],
            "start": start, "end": end, "day": start.date(), "all_day": bool(r["all_day"]), "multi_day": multi_day,
    ...
    def event_jsonld(self, s: dict) -> list[dict]:
        ...
        for o in s["upcoming"][:10]:
            ev = {
                ...
                "name": o["title"],
                "startDate": o["start"].isoformat() if not o["all_day"] else o["day"].isoformat(),
                "endDate": o["end"].isoformat() if not o["all_day"] else o["end"].date().isoformat(),
